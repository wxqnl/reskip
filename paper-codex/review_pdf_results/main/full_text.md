...
cognitive equivalence. Kahneman’s dual-process framework [18] distinguishes fast and deliberate processing, and
biological vision offers a concrete computational analogue: easy stimuli can be recognised through shallow feedforward
sweeps, while harder stimuli recruit recurrent and deeper circuits [25, 19, 10]. Recurrent vision models capture human
representational dynamics [21], and confidence-thresholded recurrence spends more steps on harder images to trade
...
decision head.
ATTNRES [24] gives a natural handle. It replaces the fixed scalar-1 residual with a learned softmax-attention over
...
• AR-RETROFIT is the main method (§2.3). A γ-gated residual injection installs ATTNRES into a frozen
pretrained transformer through a short fine-tune, using ∼0.7% new parameters and an identity-preserving
initialization.
• The installed routing signal is usable (§2.2, §3). RESKIP treats ATTNRES weights as a calibrated depth signal.
The key method insight is that routing weight alone is not a safety certificate; eligible positions must be selected
by ablation or drift.
• The retrofit transfers beyond VLM evaluation (§4). A LIBERO policy warm-started from the retrofit improves
the matched OFT baseline in our evaluated operating point, and the same routing signal supports conservative
action-stream skipping after calibration.
## 2
## Method
2.1
...
(q=0.85 default). The skip decision is a scalar comparison on a quantity already computed in phase 1; no auxiliary
network, no train-time changes, no architectural modification (cf. CALM / MoD / pruning / LayerSkip in Appendix D.3).
I(n) and A(n) are both needed because depth-attention is a routing signal, not a complete safety certificate. On our
340M from-scratch ATTNRES (Figure 2), block 2 has middle-of-the-pack importance (I=0.52) yet is catastrophic to
remove (A=8.1× PPL), while block 4 has the lowest ablation impact yet higher I than block 5. P should therefore
use both signals—low I so skip triggers often, low A so it is safe. Picking by either alone is strictly worse: {5}
alone (low-I) gives 1.14×; {3, 5} combined gives 1.19× at PPL slightly below full depth. Full position sweep in
Appendix D.4.
...
(Appendix D.2). This validates the mechanism when ATTNRES is trained in. The harder question—how to acquire it in
an already-trained model—is §2.3.
2.3
...
Table 1: RESKIP on 340M from-scratch ATTNRES. The comparison tests whether ATTNRES routing can support
calibrated dynamic depth when the architecture is trained in from scratch. The selected operating point preserves the
full-depth ATTNRES scores on the expanded task set and gives 1.19× wall-clock at sequence length 8192. Headline:
acc_norm for PIQA/HellaSwag/ARC-E/ARC-C/OpenBookQA, plain acc for MMLU/LAMBADA. Appendix D.5
gives decision-rule ablations, including static and random skip controls.
Configuration
...
over four runs; Appendix E.1). All accuracy results in this table use the fixed-shape compiled forward characterized
below.
Text
...
Table 3: Forward-pass latency on 1×H100, bf16, batch 1, median of 20 runs after 5 warmup. Eager rows:
...
perception, consistent with a router that lifts deeper reasoning paths. Text quality also lifts: +3.3pp LAMBADA /
−16% ppl on 2B and +8.7pp / −32% on 4B; HellaSwag is within ±1.1pp at both scales. Every block converges to
γn=1, so the retrofit is structurally pure ATTNRES.
The second question is attribution: are these gains caused by the ATTNRES pathway or simply by adding a small
trainable module? Parameter-matched LoRA baselines on the same data produce −0.6pp LAMBADA over four
runs (5.0pp behind the retrofit, Appendix E.1). LoRA at the same parameter scale and twice the narrow-mix step
count averages 0.526 LAMBADA versus 0.532 for the frozen base and 0.576 for the matching v1 AR-RETROFIT
run. Observer-only routing learns an α signal that never feeds back into the forward, and γ-free / interpolation
variants destabilise the frozen backbone before the router stabilises. The converged recipe therefore depends on the
identity-preserving bridge, the real forward-path ATTNRES signal, and the skip-aware surrogate loss together.
...
in Appendix E.3). The same Pareto shape reappears on action prediction (§4): RESKIP at q=0.99 is the conservative
operating point on the LIBERO action stream. We use this as a calibration test of the installed signal, not as evidence
that α alone certifies safe removal.
...
Tab. 3). Second, wrapping both base and retrofit in torch.compile (max-autotune, dynamic=False) fuses
the small router gemms. The retrofit measures 1.029× base_compiled on the same shape and compile setting,
while compile preserves accuracy (98.60% argmax agreement on LAMBADA-500, logit RMSE 0.060; Appendix E.5).
Eager-mode RESKIP contributes a further 5–9% standalone (Appendix E.3); the two paths cannot currently be composed
because dyn-skip breaks the captured CUDA graph. The systems result is therefore a two-mode characterization:
compiled full-depth retrofit for near-matched-cost accuracy, and eager calibrated skipping for additional dynamic-depth
savings.
L=4 and the v3 data mix are not arbitrary engineering choices; they are the settings that make the preceding
claims simultaneously true. L=4 sits on the accuracy plateau alongside L=2 (within 1.1pp on every VLM benchmark)
and halves the per-token router count, which is what makes the compiled-overhead result possible. Per-layer L=1
underperforms by 2–9pp; the plateau L ∈{2, 4} reproduces Kimi Team et al. [24]’s pretrain observation. The
LLaVA-OneVision-anchored v3 mix is canonical at both scales; v1 and v2 expose a binding “mix-quality” regime—v1
collapses VLM at 10k, v2 crashes diagram-reasoning at 5k. Full block-partition sweep, v1→v2→v3 trail, and γ-free /
informed-init / observer-only baselines are in Appendices E.6, C.6, C.3. The retrofit is still not free in eager mode: its
router stack adds 11–12% at L=4. The deployment claim is narrower: after both base and retrofit are compiled under
the same setting, the fixed-shape overhead falls to 2.9%. Dynamic RESKIP remains a separate eager-mode latency lever

Table 4: LIBERO 4-suite success rates (%) for Qwen3-VL-2B/4B under the three training paths. Entries are success
rates over 500 rollouts per suite (2,000 per policy). Path 0 is the stock-backbone OFT baseline; Path B warm-starts
from our canonical L=4 v3 10k VLM retrofit; Path C runs the γ-curriculum directly on VLA data without the VLM
retrofit. Clean 4B indicates a base VLM with no action-token embeddings added (Appendix F.2). Repeated-rollout
sensitivity for the 2B cells is reported in Appendix F.1. We treat the avg-column ∆’s as transfer evidence for the retrofit
warm-start rather than as broad robotics claims.
Scale
Path (steps)
Spatial
Object
...
94.55
−1.50
because thresholding currently graph-breaks torch.compile.
## 4
...
as Path B but no VLM retrofit; routers / adapters random-init, γ ramps 0→1 on VLA data). 4×H100 ZeRO-2, batch 32
effective, 30k-step default with a 60k overtraining cell. Each policy is evaluated for 500 rollouts per suite (2,000 per
policy); repeated-rollout sensitivity is reported in Appendix F.1.
4.1
LIBERO results
Path B answers the warm-start question positively. It reaches 97.15% on 2B and 96.70% on the clean-base 4B cell.
The gain concentrates on Spatial (+3.0pp at 2B) and Long-10 (+2.0pp at 4B), which are the suites where action
prediction benefits most from a stable high-level backbone. We use these results as downstream transfer evidence for
AR-RETROFIT, not as a broad robotics claim.
Path C answers the substitution question negatively. It plateaus at 94.30% at 30k, 2.85pp behind Path B; at 60k it
closes only some of the gap (95.50%, still 1.65pp behind Path B 30k). A 40k total budget (10k VLM retrofit + 30k
VLA) outperforms 60k of VLA-only training on the suites where a stable pretrained router most matters: Spatial task
6 goes from 50% (Path C 30k) and 64% (Path C 60k) to 78% (Path B 30k). Longer training is not uniformly helpful
either: Path B 60k drops −0.80pp on 2B and −0.50pp on 4B even as Path 0 on 4B keeps improving, so we use Path B
at 30k as the VLA operating point.

Table 5: RESKIP 4-suite Pareto on the trained Path B policies. Eligible blocks P and threshold τ are calibrated per
...
point: it preserves success rates while still saving 5–9% wall-clock per call. The 4B is graceful all the way to q=0.30;
2B collapses below q=0.85, consistent with the larger model carrying more depth-redundancy.
q (sim-calibrated)
Spatial
Object
Goal
...
Tab. 5 shows the 4-suite Pareto across q.
...
number of steps with γn ≡0 trainable around its zero-init (Appendix Table 6).
Table 6: Retrofit ablation on Qwen3-VL-2B. γ-free: γn trainable around 0, adapter-dominated (opposite structural
...
Table 7 collects the data-mix ablation that motivates the v3 canonical. Three intermediate cells show how the
...
Table 7: Data-mix ablation. Same γ→1 retrofit recipe (r=256, L=2, ramp-fraction per-scale) varying only the data
...
none
none
Table 9: Dynamic skip as a function of P on 340M from-scratch ATTNRES (Mmax=2, q=0.85). Picking by I alone
...
in Tab. 10; observed skip rates on the eval distribution in Tab. 11.
Table 10: Decision-rule and threshold-rule ablation on 340M from-scratch ATTNRES, lm-evaluation-harness with
...
on lm-eval data (B1.b at 3.1% observed; B2.b at 0.0%, see Tab. 11) are nearly equivalent to no-skip and are not the
...
Table 11: Observed skip rate on 8 × 512 LAMBADA tokens. The calibration set (FineWeb-Edu) and the eval set
...
fire far below the 12.5% FineWeb-Edu calibration target on the LAMBADA distribution (Tab. 11). Their near-no-skip
