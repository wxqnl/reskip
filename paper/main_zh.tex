...
本文有一个主贡献和两个支撑贡献：
\begin{itemize}
\item \textbf{\retrofit{}}（\S\ref{sec:retrofit}，\emph{主贡献}）。$\gamma$-门控残差注入通过一次较短的微调把 \attnres{} 装进一个冻结的预训练 Transformer（新增参数约 $0.7\%$、初始化时即恒等、每十亿参数仅需数十 GPU-分钟），并且每个块都收敛到 $\gamma_n{=}1$。在 Qwen3-VL-$2$B/$4$B 上，retrofit 在 $2$B 的 $6$ 个 VLM 基准中改进了 $5$ 个（另一个持平），在 $4$B 上改进了全部 $6$ 个，同时与动态 \reskip{} 组合后保持几乎一致的推理成本（\S\ref{sec:exp_retrofit}）。
...
\item \textbf{向具身控制的迁移}（\S\ref{sec:vla}）。从 retrofit 热启动的一个 LIBERO VLA 策略在 2B 与 4B 上都改进了相同条件的纯 OFT 基线；在动作流上启用 \reskip{} 并在动作分布上重新校准阈值后，在保守工作点保持训练好的策略性能（$2$B 上 $+1.10$pp；$4$B 上落在种子噪声范围内）。
\end{itemize}
...
\begin{equation}
  ...
  \label{eq:gamma_gate}
\end{equation}
$\mathrm{Block}_n$ 是预训练块，原封不动；\attnres{} 进入是\emph{在块之间}，作为一个修正项。retrofit 参数 $\{\mathbf{w}_n, \gamma_n, A_n\}$ 在 $r{=}256, N{=}14$ 的 2B 上约 $15$M（约基础模型 $0.7\%$）；整个基础模型——嵌入、视觉塔、解码器层、LM 头——都被冻结。
...
第一个实证问题是：retrofit 是否在不简单牺牲预训练模型能力的前提下解决了安装问题。我们在 $L{=}4$、$N{=}7$ 的块划分上对 \textbf{Qwen3-VL-2B}~\citep{bai2025qwen3vl}（$L{=}28, d{=}2048$）做 retrofit，并在 $L{=}4$、$N{=}9$ 的块划分上对 \textbf{Qwen3-VL-4B}（$L{=}36, d{=}2560$）做 retrofit（块划分扫见附录~\ref{app:block_partition}）。$L{=}4$ 是两种规模的标准划分：它与 $L{=}2$ 同处准确率平台，且把每 token 的路由器调用数减半，而后者是下文推理成本结果的主导因素。所有基础参数都被冻结；在 $r{=}256$ 下，$2$B 的 retrofit 参数约 $15$M、$4$B 约 $23.7$M，按 Eq.~\ref{eq:retrofit_loss} 训练。我们对比两种数据混合：\textbf{v1}（$50$/$50$ UltraChat-$200$k~\citep{ding2023ultrachat} + LLaVA-Instruct-VSFT~\citep{liu2023llava}，$5$k 步；$2$B 上约 $22$ GPU-分钟），作为可比 LoRA 的窄混合对照；以及 \textbf{v3}（$60\%$ LLaVA-OneVision-Data~\citep{li2024llavaonevision} + $20\%$ UltraChat + $10\%$ NuminaMath + $10\%$ OpenThoughts，$10$k 步），是我们所有报告结果使用的标准配方。评估使用 LAMBADA~\citep{paperno2016lambada}/HellaSwag~\citep{zellers2019hellaswag} 评估文本，使用六个 \texttt{lmms-eval} 基准评估 VLM（MMBench~\citep{liu2023mmbench}、MMMU~\citep{yue2024mmmu}、MMStar~\citep{chen2024mmstar}、AI2D~\citep{kembhavi2016ai2d}、OCRBench~\citep{liu2024ocrbench}、RealWorldQA~\citep{grok2024realworldqa}）；文本侧 \reskip{} 的可跳块集合按按块静态移除选取，VLA 的可跳块集合则按训练好的策略上的动作漂移扫选取（附录~\ref{app:block_removal}, \ref{app:vla_pareto}）。训练前已验证初始恒等（$\gamma{=}0$ 时复现基础模型；bf16 下 $|\Delta\text{logits}|$ 最大 $0.375$，argmax $100\%$ 一致；附录~\ref{app:identity}）。
...
第三个问题是：装好的信号是否可用于深度分配。在按规模选取的可跳块集合（$2$B：$\mathcal{P}{=}\{1,4\}$；$4$B：$\mathcal{P}{=}\{1,2\}$）和按块校准的阈值（$32$ 个前缀留出集上 $w_{\text{recent}}$ 的第 $q$ 分位数）下，动态跳过在保守端保持 LAMBADA（$q{=}0.85$ 在无跳过 $1.0$pp 之内；完整 $(q,M)$ 扫见附录~\ref{app:retrofit_pareto}）。同样的 Pareto 形状在动作预测上重新出现（\S\ref{sec:vla}）：在同一评估扫中，$q{=}0.99$ 下 \reskip{} 在 $2$B 上 $+1.10$pp，在 $4$B 上与无跳过的四套件均值持平（在种子噪声范围内），验证了同一阈值校准协议下深度轴分配从 token 级到动作级解码的迁移性。
...
Path B 在两种规模上都肯定地回答了热启动问题：$2$B 上 $96.75\%$（两个重复 rollout 套件按种子取均；附录~\ref{app:vla_variance}），干净 $4$B 基础上 $96.70\%$，在四套件聚合上分别比 Path 0 改进 $+0.70$pp / $+0.65$pp。$2$B 的增益由 Spatial 主导（$+3.0$pp），其余三个套件落在与 Path 0 的种子噪声范围内；$4$B 的增益由 Long-10 主导（$+2.0$pp），那是动作分块深度最关键的套件。我们的解读不是 \retrofit{} 解决了 VLA，而是：经过 retrofit 的深度路由骨干能在不损害控制的前提下迁移过来，并在那些依赖骨干质量的套件上提供有利的热启动。
...
\begin{table}[t]
\caption{\textbf{训练好的 Path B 策略上的 \reskip{} 4 套件 Pareto}。可跳块 $\mathcal{P}$ 与阈值 $\tau$ 按规模在动作分布上校准；$q$ 是产生 $\tau$ 的经验分位。无跳过参考行来自同一次 \reskip{} 校准/评估扫。保守端（这里 $q{=}0.99$）是推荐工作点：$2$B 上 $+1.10$pp（四个套件方向一致），$4$B 上与无跳过基线持平（按套件波动落在种子噪声内；总体 $+0.20$pp），同时每次调用节省 $5$--$9\%$ wall-clock。$4$B 一直到 $q{=}0.30$ 都很从容（仅 $-4.4$pp）；$2$B 在 $q{=}0.85$ 以下崩塌，与 ``更大模型携带更多深度冗余'' 一致。}
...
\retrofit{} 通过一次较短的微调把一种内在的深度路由信号装进一个冻结的预训练 Transformer；得到的路由权重作为自然应用驱动 \reskip{}。在标准的 $L{=}4$ v3 配方下，retrofit 在 Qwen3-VL 上 $2$B 改进 $6$ 个 VLM 基准中的 $5$ 个、$4$B 上 $6$ 个全部改进，并保持与动态 \reskip{} 组合下的近一致推理成本，同时把 LAMBADA 抬高 $+3.3$/$+8.7$pp，最大单格增益出现在 MMStar math（这看起来是一种向逐步推理路径的选择性重权，而不是均匀的推理能力提升）。装好的信号是可用的：在 $340$M 从零模型上，\reskip{} 在零基准下降下达到 $1.19\times$ wall-clock，跳过率匹配 $\sim$$12\%$ 的对照下，输入相关规则严格优于静态/随机调度。从 retrofit 热启动的 LIBERO VLA 在 2B 与 4B 上都改进了相同条件的纯 OFT 基线；动作流上以动作分布重新校准阈值的 \reskip{} 在保守工作点保持训练好的策略，并在 $2$B 上带来 $+1.10$pp 的提升。
...
\emph{(ii)~math-CoT 文本上限 $\leq 20\%$。} v2 在 2B 上 $40\%$ math-CoT 占比把 AI2D 砸到 $0.283$——retrofit 的路由器在文本侧呈现强符号推理梯度时学会了远离视觉密集路径。v3 把 math-CoT 控制在 $20\%$ 并把 AI2D 恢复到 $0.765$。
