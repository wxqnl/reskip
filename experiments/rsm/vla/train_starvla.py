...
gradient_accumulation_steps = int(
    os.environ.get("STARVLA_GRADIENT_ACCUMULATION_STEPS", "1")
)
if gradient_accumulation_steps < 1:
    raise ValueError("STARVLA_GRADIENT_ACCUMULATION_STEPS must be positive")
accelerator = Accelerator(
    deepspeed_plugin=deepspeed_plugin,
    gradient_accumulation_plugin=GradientAccumulationPlugin(
        num_steps=gradient_accumulation_steps,
        # ZeRO-2 partitions gradients and rejects DeepSpeedEngine.no_sync().
        # DeepSpeed already owns the actual accumulation boundary in
        # engine.backward()/engine.step(), so keep synchronization enabled on
        # every micro-batch while Accelerate tracks the optimizer-step cadence.
        ...
        if self.accelerator.is_main_process:
            ...
            for key, value in output_dict.items():
                ...
                if isinstance(value, numbers.Number):
                    step_metrics[f"eval_{key}"] = float(value)
                elif isinstance(value, torch.Tensor) and value.ndim == 0:
                    step_metrics[f"eval_{key}"] = float(value.item())
