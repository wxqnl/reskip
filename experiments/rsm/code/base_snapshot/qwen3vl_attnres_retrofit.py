...
class BlockAttnResRouter(nn.Module):
    def __init__(
        ...
        if identity_anchored_source_calibration:
            # Only sources that can become historical need a calibration row.
            # The most recent source is deliberately never calibrated, keeping
            # alpha_recent=1 an exact identity anchor for every target block.
            self.source_calibration = nn.Parameter(
                torch.zeros(max(num_sources - 2, 0), hidden_size)
            )
        else:
            self.register_parameter("source_calibration", None)
    ...
    def _route_impl(
        ...
        if self.source_calibration is not None and values.shape[0] > 1:
            historical_count = values.shape[0] - 1
            scale = torch.tanh(
                self.source_calibration[:historical_count]
            ).to(values.dtype)
            calibrated_historical = values[:-1] * (
                1.0 + scale[:, None, None, :]
            )
            values = torch.cat((calibrated_historical, values[-1:]), dim=0)
...
class Qwen3VLAttnResRetrofit(nn.Module):
    ...
    With ``identity_anchored_source_calibration``, historical residual sources
    receive a zero-initialized per-channel calibration before native AttnRes
    key/value routing. The recent source remains untouched.
    """
    ...
    def __init__(
        ...
        self.router = BlockAttnResRouter(
            ...
            identity_anchored_source_calibration=(
                identity_anchored_source_calibration
            ),
        )
        self.identity_anchored_source_calibration = bool(
            identity_anchored_source_calibration
        )
        ...
        if compute_native_features:
            ...
            native_features = {
                **router_native_features,
                ...
                # Identity default for ordinary AttnRes checkpoints. The RSG
                # installer replaces this value with its learned token factor.
                "residual_strength_factor": torch.ones_like(
                    alpha[..., -1], dtype=torch.float32
                ),
            }
        ...
        **kwargs,
        ...
        for block_idx in range(self.num_blocks):
            ...
            native_feature_request = (
                ...
                or inference_native_feature_request
            ...
            (
                ...
                return_fused_native_inputs=fused_joint_native_request,
            )
            ...
            if inference_native_feature_request and native_features is not None:
