"""Apply the final frozen seed-2 capped-policy screen rule."""
...
SCREEN = ROOT / "reskip/qwen3vl_4b_seed2_capped_screen"
...
def result(variant: str) -> dict:
    ...
    scores = {task: float(data[task][key]) * scale for task, (key, scale) in SCORE_KEYS.items()}
    return {"variant": variant, "scores": scores, "macro": statistics.fmean(scores.values()), "result_json": str(path)}


def main() -> None:
    full = result("full")
    candidate = result("capped_native")
    stats_path = SCREEN / "capped_native/skip_stats.json"
    ...
    passed = delta >= -0.30 and 0.65 <= block_equiv <= 1.15
    payload = {
        "status": "continue_to_seed2_formal" if passed else "stop_final_policy_failed",
        "full": full,
        "candidate": candidate,
        "delta_vs_full_pp": delta,
        "block_equivalents_per_token": block_equiv,
        "screen_rule": {"minimum_delta_pp": -0.30, "block_equivalent_interval": [0.65, 1.15]},
        "policy_json": str(ROOT / "reskip/qwen3vl_4b_seed2_capped_policy_seed259123/capped_native_policy.json"),
        "formal_seed2_output_unseen": True,
    }
