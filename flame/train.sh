...
if [[ -z "${path}" ]]; then
  path=$(grep -oP '(?<=--checkpoint.folder )[^ ]+' <<< "$params" || true)
fi
if [[ -z "${path}" ]]; then
  if [ "${date:-}" == "" ]; then
    date=$(date +%Y%m%d%H%M)
  fi
  path="exp/manual-run-${date}"
fi
steps=$(grep -oP '(?<=--training.steps )[^ ]+' <<< "$params")
config=$(grep -oP '(?<=--model.config )[^ ]+' <<< "$params")
tokenizer=$(grep -oP '(?<=--model.tokenizer_path )[^ ]+' <<< "$params")
model=$(
  python -c "import fla, sys, custom_models; from transformers import AutoConfig; print(AutoConfig.from_pretrained(sys.argv[1], trust_remote_code=True).model_type)" "$config"
)

mkdir -p "$path"
for item in train.sh pyproject.toml README.md LICENSE configs custom_models flame scripts; do
  if [[ -e "$item" ]]; then
    cp -r "$item" "$path"/
  fi
done
if [[ -d 3rdparty/flash-linear-attention/fla ]]; then
  mkdir -p "$path"/3rdparty/flash-linear-attention
  cp -r 3rdparty/flash-linear-attention/fla "$path"/3rdparty/flash-linear-attention/
fi
if [[ -d 3rdparty/torchtitan/torchtitan ]]; then
  mkdir -p "$path"/3rdparty/torchtitan
  cp -r 3rdparty/torchtitan/torchtitan "$path"/3rdparty/torchtitan/
fi
